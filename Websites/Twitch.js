...
function setup() {
  ...
  twitchInfoHandler.readyCheck = function () {
    return (
      document.getElementsByClassName("video-player__default-player").length >
        0 && document.getElementsByTagName("video").length > 0
    );
  };
  ...
  twitchInfoHandler.title = function () {
    return document.querySelector("h2.CoreText-sc-1txzju1-0").title;
  };
  ...
  twitchInfoHandler.cover = function () {
    return document
      .querySelector("figure.tLGH > img:nth-child(1)")
  ...
  twitchInfoHandler.duration = function () {
    ...
    if (document.getElementsByTagName("video")[0].duration == "1073741824") {
      var duration_read = document
        .querySelector("span.live-time")
        .innerText.split(":");
